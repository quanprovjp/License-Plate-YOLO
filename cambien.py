...
def detect_license_plate(camera_id, output_file, ser=None, is_entry=True, model=None, reader=None):
    ...
    camera_id: Index của camera (0 cho camera vào, 1 cho camera ra)
    output_file: Đối tượng file để ghi biển số
    is_entry: True nếu là camera lối vào, False nếu là camera lối ra
    model: Mô hình YOLO đã tải
    reader: Đối tượng EasyOCR đã tải
    """
    ...
    detection_delay = 2  # Giảm xuống 1 giây để phản hồi nhanh hơn
    plate_detected = False
    timeout = 30  # Thời gian tối đa để nhận diện (giây)
    start_time = time.time()
    process_interval = 0.2  # Xử lý mỗi 0.2 giây
        ...
        while time.time() - start_time < timeout:
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    if conf > 0.3:  # Giảm ngưỡng từ 0.5 xuống 0.3
                        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                        plate = frame[y1:y2, x1:x2]
                        plate = cv2.resize(plate, (200, 50))
                        plate_gray = cv2.cvtColor(plate, cv2.COLOR_BGR2GRAY)
                        plate_gray = cv2.equalizeHist(plate_gray)  # Tăng độ tương phản
            ...
            if plate_detected and last_plate_text and (time.time() - last_detection_time) >= detection_delay:
                ...
                if is_entry:
                    # Camera vào: Thêm biển số vào queue và file
                    ...
                    # Camera ra: Đối chiếu và xóa biển số
                    plate_matched = False
                    temp_queue = queue.Queue()
                    while not license_plate_queue.empty():
                        plate, timestamp = license_plate_queue.get()
                        if datetime.now() - timestamp > timedelta(hours=1):
                            continue  # Bỏ qua biển số quá cũ
                        if plate == last_plate_text:
                            logging.info(f"Biển số {last_plate_text} khớp, mở barrier ra")
                            print(f"Biển số {last_plate_text} khớp, mở barrier ra")
                            plate_matched = True
                        else:
                            temp_queue.put((plate, timestamp))
                    # Khôi phục các biển số không khớp
                    ...
                    if plate_matched and ser and ser.is_open:
                        ...
                        logging.warning(f"Biển số {last_plate_text} không khớp")
                        print(f"Biển số {last_plate_text} không khớp")
            ...
            try:
                if ser.in_waiting > 0:
                    ...
                    if command == "CAR_ENTRY":
                        ...
                        detect_license_plate(1, output_file, ser, True, model, reader)  # Camera USB 1 cho lối vào
                    elif command == "CAR_EXIT":
                        logging.info("Phát hiện xe ở lối ra, kích hoạt camera 1...")
                        print("Phát hiện xe ở lối ra, kích hoạt camera 1...")
                        detect_license_plate(0, output_file, ser, False, model, reader)  # Camera USB 2 cho lối ra
